          ...
          {latestProjects.map((project, index) => (
            <div key={index} className="relative overflow-hidden">
              <Link to={`/works/${index + 1}`}>
                ...
                />
                <motion.div
                  className="absolute inset-0 flex flex-col items-center justify-center text-center bg-[rgba(0,0,0,0.7)] opacity-0 transition-opacity duration-1000"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  whileHover={{ opacity: 0, scale: 1.3 }}
                  
              whileTap={{ scale: 0.95 }}
                >
                  <h3 className="mb-2 text-2xl font-bold text-[#fee57e] md:text-4xl">{project.title}</h3>
                  <p className="text-xl text-center text-white md:text-2xl max-w-80 md:max-w-96">{project.description}</p>
